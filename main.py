...
class SecretSanta(Tk):
    def __init__(self):
        ...
        Label(self.frame, text="Send After (seconds):").grid(column=0, row=9, sticky="w")
        self.send_after_entry = Entry(self.frame)
        self.send_after_entry.grid(column=1, row=9, padx=10, pady=5, columnspan=2)

        # Create a Label with the name Check for Reminder.
        Label(self.frame, text="Check for Reminder:").grid(column=0, row=10, sticky="w")
        self.recurring_email_checkbox = Checkbutton(self.frame, variable=self.is_reminder)
        self.recurring_email_checkbox.grid(column=1, row=10, padx=10, pady=5, columnspan=2)
    ...
    def send_assignments(self):
        ...
        if((len(self.assignments) == 0 and is_reminderset) or not is_reminderset):
